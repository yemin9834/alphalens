#!/usr/bin/env python3
"""
Package the Orchestrator Lambda using Docker (linux/amd64).
...
def package_lambda() -> Path:
    ...
    backend_dir = orchestrator_dir.parent
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print("Creating Orchestrator Lambda package (Alex planner pattern)...")
        ...
        for filename in AGENT_FILES:
            shutil.copy(orchestrator_dir / filename, package_dir / filename)
        ...
        zip_path = orchestrator_dir / "orchestrator_lambda.zip"
        if zip_path.exists():
            zip_path.unlink()
